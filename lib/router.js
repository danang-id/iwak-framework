	...
	*/
	Router.prototype.resource = function(config, paramA, paramB, paramC) {
		try {
			...
			const crudMethods = Object.keys(crud);

			for(var i = 0, len = crudMethods.length; i < len; i++) {
				if(loaded.controller[crudMethods[i]]) {
					this.hasRoute = true;
					...
					if(!loaded.insMiddlewares[method]) {
						if(loaded.middlewares) 
						  this.router[crud[method].method](prefix, loaded.middlewares, loaded.controller[method]);
						else 
						  this.router[crud[method].method](prefix, loaded.controller[method]);
					}
					else {
						let nMiddlewares = (loaded.middlewares) ? loaded.insMiddlewares[method].concat(loaded.middlewares) : loaded.insMiddlewares[method];
						this.router[crud[method].method](prefix, nMiddlewares, loaded.controller[method]);
					}
				}
			}
